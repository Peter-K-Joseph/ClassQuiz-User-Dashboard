...
function updateSerices() {
    ...
    for (let i = 0; i < userData.Quizzes.total; i++) {
        let dateToDisplay: string, quizStat: string, quizStattoDisplay: string;
...
class AlertBox {
    title: string | null;
    content: string | null;
    AcceptButtonFunctionCall: any;
    RejectButtonFunctionCall: any;
    iconOnDisplay: any;
    customModalID: string;
    displayAcceptButton: boolean;
    displayRejectButton: boolean;
    value_to_display_in_accept_button: string | null;
    value_to_display_in_reject_button: string;
    icons: any;
    constructor(title: string | null, content: string | null, Call_this_when_accept_pressed: string | null, Call_this_when_reject_pressed: string | null, Show_this_favicon_in_the_modal: string | "alert" | "danger" | "radioactive" | "info" | "question" | null, customID: string | null, displayAcceptButton: boolean, displayRejectButton: boolean, value_to_display_in_accept_button: string | null, value_to_display_in_reject_button: string | null) {
        let icons = {
...
class ActiveModal {
    ...
    constructor(modalType: string, modalID: string, modalParentClass: string, modalClose: number | null, method: "open-fullPage" | "open-settings" | "show-alert" | null, alertBox_title: string | null, alertBox_context: string | null, alertBox_class) {
    ...
    modalAction(method: "open-fullPage" | "open-settings" | "show-alert" | null, alertBox_title: string, alertBox_context: string, alertBox_class) {
        switch (method) {
            case "open-fullPage":
...
function alertMode() {
    let cache: string[] = ["Heading", "Context", "Function"];
...
function swtichQuizState(input: boolean, jsonID: number) {
    console.log(jsonID);
...
for (let i = 0; i < document.getElementsByClassName("settingOption").length; i++) {
    ...
    classSelect[i].addEventListener("click", () => {
        ...
        (userData.DarkTheme === true) ? (<HTMLElement>document.querySelector(".selectedNav")).style.filter = "invert(1)" : (<HTMLElement>document.querySelector(".selectedNav")).style.filter = null;
    })
}
